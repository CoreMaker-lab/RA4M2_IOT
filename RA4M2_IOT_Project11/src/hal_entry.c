...
static void OLED_DrawPage_TUYA(void)
{
    ...
    if (++g_tuya_num >= 100)
    {
        ...
        /* ─────────────────── OLED 行标签 ────────────────────────── */
        OLED_ShowString(0,  48, (u8 *)"ADC:", 16, 1);   // 第  6 行左侧：显示 “ADC:”
        OLED_ShowString(64, 48, (u8 *)"VOL:", 16, 1);   // 第  6 行偏右：显示 “VOL:”

        /* ─────────────────── ADC 采样并换算 ─────────────────────── */
        uint16_t adc_data13 = 0;     // 存放通道 AN013 原始 12-bit 数据
        double   a13;                // 转换后的电压值 (单位 V)
        uint32_t t100;               // 小数两位 ×100 的临时变量

        R_ADC_ScanStart(&g_adc0_ctrl);     /* 启动一次软件触发扫描            */
        scan_complete_flag = false;        /* 在 ADC 回调中置位                */
        ...
        assert(FSP_SUCCESS == err);
        /* 将 0-4095 映射到 0-3.3 V ---------------------------------- */
        a13=(double)(adc_data13/4095.0)*3.3;
//        printf("P014(AN12)=%d,voltage=%f\n",adc_data13,a13);
        /* ─────────────────── OLED 数值显示 ─────────────────────── */
        /* ADC 原码：4 位十进制，例如 0000–4095                       */
        OLED_ShowNum(32, 48, adc_data13, 4, 16,1);

        /* 电压整数位：0–3（3.3 V 情况下最大为 3）                    */
        OLED_ShowNum (96, 48,(uint32_t)fabs(a13),1,16, 1);
        /* 小数点 '.'                                                 */
        OLED_ShowChar (104, 48, '.',16, 1);
        /* 小数后两位：把电压 ×100 再取低 2 位                         */
        t100=(uint32_t)(fabs(a13)*100);
        OLED_ShowNum (112, 48,(uint32_t)t100%100,2,16, 1);
